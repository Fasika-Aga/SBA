function calculateWeightedAverage(assignments, submissions) {
  let totalPoints = 0;
  let weightedPoints = 0;

  assignments.forEach((assignment) => {
    const submission = submissions.find((sub) => sub.assignment_id === assignment.id);

    if (submission && submission.submitted_at <= assignment.due_at) {
      const latePenalty = submission.submitted_at > assignment.due_at ? 0.9 : 1;
      const score = submission.score * latePenalty;
      totalPoints += assignment.points_possible;
      weightedPoints += score;
    }
  });

  return totalPoints === 0 ? 0 : (weightedPoints / totalPoints) * 100;
}

function getLearnerData(courseInfo, assignmentGroup, learnerSubmissions) {
  try {
    // Validate if AssignmentGroup belongs to the course
    if (assignmentGroup.course_id !== courseInfo.id) {
      throw new Error("Invalid input: AssignmentGroup does not belong to the course.");
    }

    const learnerData = {
      id: learnerSubmissions[0].learner_id,
      avg: 0,
    };

    const assignmentScores = {};

    assignmentGroup.assignments.forEach((assignment) => {
      // Handle potential errors in data
      if (assignment.points_possible === 0 || isNaN(assignment.points_possible)) {
        throw new Error(`Invalid data: points_possible for assignment ${assignment.id} is 0 or not a number.`);
      }

      const submission = learnerSubmissions.find((sub) => sub.assignment_id === assignment.id);

      if (submission && submission.submission) {
        const percentageScore = (submission.submission.score / assignment.points_possible) * 100;
        assignmentScores[assignment.id] = percentageScore;
      }
    });

    learnerData.avg = calculateWeightedAverage(assignmentGroup.assignments, learnerSubmissions);
    learnerData.assignments = assignmentScores;

    return [learnerData];
  } catch (error) {
    console.error(error.message);
    return [];
  }
}



// The provided course information.
const CourseInfo = {
  id: 451,
  name: "Introduction to JavaScript"
};

// The provided assignment group.
const AssignmentGroup = {
  id: 12345,
  name: "Fundamentals of JavaScript",
  course_id: 451,
  group_weight: 25,
  assignments: [
    {
      id: 1,
      name: "Declare a Variable",
      due_at: "2023-01-25",
      points_possible: 50
    },
    {
      id: 2,
      name: "Write a Function",
      due_at: "2023-02-27",
      points_possible: 150
    },
    {
      id: 3,
      name: "Code the World",
      due_at: "3156-11-15",
      points_possible: 500
    }
  ]
};

// The provided learner submission data.
const LearnerSubmissions = [
  {
    learner_id: 125,
    assignment_id: 1,
    submission: {
      submitted_at: "2023-01-25",
      score: 47
    }
  },
  {
    learner_id: 125,
    assignment_id: 2,
    submission: {
      submitted_at: "2023-02-12",
      score: 150
    }
  },
  {
    learner_id: 125,
    assignment_id: 3,
    submission: {
      submitted_at: "2023-01-25",
      score: 400
    }
  },
  {
    learner_id: 132,
    assignment_id: 1,
    submission: {
      submitted_at: "2023-01-24",
      score: 39
    }
  },
  {
    learner_id: 132,
    assignment_id: 2,
    submission: {
      submitted_at: "2023-03-07",
      score: 140
    }
  }
];

function getLearnerData(course, ag, submissions) {
  // here, we would process this data to achieve the desired result.
  const result = [
    {
      id: 125,
      avg: 0.985, // (47 + 150) / (50 + 150)
      1: 0.94, // 47 / 50
      2: 1.0 // 150 / 150
    },
    {
      id: 132,
      avg: 0.82, // (39 + 125) / (50 + 150)
      1: 0.78, // 39 / 50
      2: 0.833 // late: (140 - 15) / 150
    }
  ];

  return result;
}

const result = getLearnerData(CourseInfo, AssignmentGroup, LearnerSubmissions);

console.log(result);
